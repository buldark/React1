...
const Test01 = () => {
    ...
    //                여기 글자 아무거나 상관없음.  : 이벤트 발생 시 값을 가져오는ㄴ....
    const onChangeId = (e) => {
        //비구조 할당 으로 input 태그안의 모든 속성중에서 value만 가져온다.
        const {value} =e.target//이벤트가 발생한 대상 비구조 분해 /  e.target은 이벤트 발생지 제공되는거임 내가 만든것이 아닌.이벤트가 발새한 대상  
        setId(value)
    ...
    const onReset = () =>{
        setId('')
        setPwd('')
        idRef.current.focus() //이럼 이쪽에 포커스가 들어감 
    }
    return (
        <div>
            ...
            <br/>
            {/*      비밀번호가 6자리보다 작으면 disabled / 즉 6자리부터 활성화 */}
